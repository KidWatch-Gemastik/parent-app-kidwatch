...
import { Loader2, PhoneIncoming, PhoneOutgoing, PhoneMissed, PhoneOff } from "lucide-react"
            ...
            <div className="relative z-10 flex">
                ...
                <main className="flex-1 p-6 space-y-8">
                    ...
                    <section className="p-6 max-w-5xl mx-auto">
                        <h1 className="text-2xl font-bold text-emerald-400 mb-6">Riwayat Panggilan</h1>

                        {loading ? (
                            <div className="flex items-center justify-center py-20 text-gray-400">
                                <Loader2 className="w-6 h-6 animate-spin mr-2" /> Memuat log panggilan...
                            </div>
                        ) : logs.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-24 text-gray-400 space-y-4">
                                ...
                                <p className="text-sm text-gray-500 max-w-sm text-center">
                                    Semua riwayat panggilan anak Anda akan tampil di sini setelah aplikasi mulai merekam log panggilan.
                                </p>
                            </div>
                        ) : (
                            <div className="grid sm:grid-cols-2 gap-6">
                                {logs.map((log) => (
                                    <Card
                                        key={log.id}
                                        className="bg-gray-900/70 border-gray-800 hover:bg-gray-900/90 transition-colors duration-300 rounded-xl shadow-lg"
                                    >
                                        ...
                                        <CardContent className="text-gray-400 text-sm space-y-2">
                                            <div className="flex justify-between">
                                                <span>Anak:</span>
                                                <span className="text-white">{log.children?.name || "Tidak diketahui"}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span>Durasi:</span>
