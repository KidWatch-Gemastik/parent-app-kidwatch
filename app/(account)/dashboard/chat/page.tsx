...
import { ChevronLeft, ChevronRight, LayoutDashboard, MessageCircle } from "lucide-react"
...
export default function ChatPage() {
    ...
    const filteredList = useMemo(() => {
        return list.filter((child) => child.child_name.toLowerCase().includes(search.toLowerCase()))
    }, [list, search])
    ...
    return (
        <div className="flex h-[calc(100vh-2rem)] rounded-lg overflow-hidden bg-gradient-to-br from-slate-950 via-gray-950 to-emerald-950 relative">
            ...
            {isMobile && !showSidebar && (
                <div className="absolute inset-0 bg-black/50 z-10 md:hidden" onClick={() => setShowSidebar(true)} />
            )}
            ...
            <div
                className={cn(
                    ...
                    // Mobile behavior
                    isMobile && (showSidebar ? "w-full" : "w-0 overflow-hidden"),
                    // Mobile positioning
                    ...
                    {!loading && filteredList.length === 0 && (
                        <div className="flex items-center justify-center h-full p-4">
                            <div className="flex flex-col items-center text-center opacity-60">
                                ...
                                />
                                {!collapsed && (
                                    <p className="text-sm text-gray-400">{search ? "Tidak ada hasil" : "Anak Belum di Hubungkan"}</p>
                                )}
                            </div>
                        </div>
                    )}
                                    ...
                                    {!collapsed && (
                                        <div className="flex-1 overflow-hidden min-w-0">
                                            <div className="flex justify-between items-center mb-1">
                                                <span className="font-medium text-white truncate">{child.child_name}</span>
                                                {child.last_time && (
                ...
                {selectedChild ? (
                    ...
                    <div className="flex-1 flex items-center justify-center opacity-70 p-4">
                        <div className="flex flex-col items-center gap-6 text-center">
                            ...
                            />
                            <h6 className={cn("font-semibold text-emerald-200", isMobile ? "text-lg" : "text-xl")}>
