...
export default function AIAssistant() {
  ...
  const askAI = async (q?: string) => {
    ...
    try {
      ...
      const answer =
        data.answer || "❌ Maaf, saya tidak bisa menjawab saat ini.";
      ...
      case "audio":
        return (
          <audio
            src={msg.file_url}
            controls
            className={cn("w-full", isMobile ? "max-w-20" : "max-w-32")}
          />
        );
      case "location":
        return (
  ...
  const handleTextareaChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    textarea.style.height =
      Math.min(textarea.scrollHeight, isMobile ? 100 : 120) + "px";
  };
      ...
      <div className="flex h-full max-w-7xl mx-auto overflow-y-auto gap-4 p-4">
        <aside
          className={cn(
            ...
            isMobile &&
              (showSidebar
                ? "fixed left-4 top-20 bottom-4 w-80 z-50 shadow-2xl"
                : "hidden")
          )}
        >
                ...
                {Object.entries(groupedChatSessions).map(
                  ([dateLabel, sessions]) => {
                    ...
                    const displaySessions = Object.values(
                      (sessions as ChatSession[]).reduce<
                    ...
                    return (
                      <div
                        key={dateLabel}
                        className="border-b border-emerald-500/20 pb-3"
                      >
                        {/* Header tiap tanggal */}
                            ...
                            {displaySessions.map((session, i) => (
                              ...
                              >
                                <div className="flex items-start gap-2">
                                  <span className="text-emerald-400 shrink-0 text-sm">
                                    🧑
                                  </span>
                                  <div className="flex-1 min-w-0">
                                    <span className="line-clamp-2 leading-relaxed group-hover:text-gray-200 transition-colors">
                                      {session.userQuestion.slice(
                                        0,
                                        isMobile ? 45 : 55
                                      )}
                                      {session.userQuestion.length >
                                        (isMobile ? 45 : 55) && "..."}
                                    </span>
            ...
            {chatMedia.length === 0 ? (
              <div className="text-center py-6 bg-gray-900/50 rounded-xl border border-emerald-500/20">
                ...
                <p className="text-gray-400 text-sm">
                  Belum ada media terkirim dari anak.
                </p>
              </div>
            ) : (
