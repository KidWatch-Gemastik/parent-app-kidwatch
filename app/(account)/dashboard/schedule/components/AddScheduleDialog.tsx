...
import type { Scheduling, DayOfWeek, Urgency, Recurrence, Status } from "@/types"
import { useSupabase } from "@/providers/SupabaseProvider"
import Image from "next/image"

interface AddScheduleDialogProps {
    onAdd: (schedule: Omit<Scheduling, "id" | "created_at" | "updated_at">) => void
...
export default function AddScheduleDialog({
    ...
    useEffect(() => {
        const fetchChildren = async () => {
            ...
            if (error) console.error("Gagal ambil anak:", error.message)
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (scheduleToEdit && onEdit) {
            onEdit({
                ...scheduleToEdit,
                ...formData,
                notify_before: formData.notify_before === "none" ? null : formData.notify_before || null,
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            ...
            <SheetContent className="sm:max-w-[600px] md:max-w-[700px] lg:max-w-[800px] overflow-y-auto" side={'right'}>
                <SheetHeader>
                    <SheetTitle>{scheduleToEdit ? "Edit Jadwal" : "Tambah Jadwal Baru"}</SheetTitle>
                </SheetHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-6 px-4 py-2">
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            <SelectTrigger className="w-full">
                                <SelectValue placeholder={loadingChildren ? "Loading..." : "Pilih anak"} />
                            </SelectTrigger>
                            <SelectContent>
                                {children.map((child) => (
                                    <SelectItem key={child.id} value={child.id} className="flex items-center gap-2">
                                        <Image
                                            unoptimized
                                            width={24}
                                            height={24}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => setFormData((prev) => ({ ...prev, recurrence: value as Recurrence }))}
                            >
                                <SelectTrigger className="w-full">
                                    <SelectValue />
                                </SelectTrigger>
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => setFormData((prev) => ({ ...prev, notify_before: value }))}
                            >
                                <SelectTrigger className="w-full">
                    ...
                    <div className="flex justify-end gap-3 pt-4">
                        ...
                        <Button type="submit">{scheduleToEdit ? "Simpan Perubahan" : "Simpan Jadwal"}</Button>
                    </div>
                </form>
            </SheetContent>
        </Sheet>
