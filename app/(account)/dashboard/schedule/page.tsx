...
function useSchedules() {
    ...
    const handleAddSchedule = useCallback(async (newSchedule: Omit<Scheduling, "id" | "created_at" | "updated_at">) => {
        try {
            const { data, error } = await supabase
                .from("scheduling")
                .insert([{ ...newSchedule }])
    ...
    const handleEditSchedule = useCallback(async (schedule: Scheduling) => {
        try {
            const { data, error } = await supabase
                .from("scheduling")
                .update(schedule)
...
export default function SchedulePage() {
    ...
    const [scheduleToDelete, setScheduleToDelete] = useState<Scheduling | null>(null)
    ...
    const { schedules, loading, error, handleAddSchedule, handleEditSchedule, handleDeleteSchedule } = useSchedules()
    ...
    const filteredSchedules = useMemo(() => {
        return schedules.filter((schedule) => {
            if (filterDay !== "all" && schedule.day_of_week !== filterDay) return false
    ...
    const stats = useMemo(() => {
        ...
        const thisWeek = schedules.filter((s) => s.recurrence === "weekly" || s.recurrence === "daily").length
                ...
                <main className="flex-1 p-4 md:p-6 space-y-6 md:ml-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground">Semua aktivitas terjadwal</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                ...
                                <Badge className="bg-accent text-accent-foreground">{stats.active}</Badge>
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-bold text-accent">{stats.active}</div>
                                <p className="text-xs text-muted-foreground">Aktivitas yang sedang berjalan</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground">Aktivitas rutin mingguan</p>
                            </CardContent>
                        </Card>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                        <div className="flex flex-wrap gap-3">
                            <Select value={filterDay} onValueChange={(value) => setFilterDay(value as DayOfWeek | "all")}>
                    ...
                    {loading ? (
                        <p className="text-center text-muted-foreground">Memuat jadwal...</p>
                    ) : error ? (
                            ...
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-muted-foreground text-pretty">
                                        Mulai dengan menambahkan jadwal aktivitas pertama untuk anak Anda.
                                    </p>
                                </div>
                                <Button onClick={() => {
                ...
                <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                    <DialogContent>
                        ...
                        <p className="py-4">
                            ...
                            <strong>{scheduleToDelete ? getDayInIndonesian(scheduleToDelete.day_of_week) : ""}</strong>?
                        </p>
