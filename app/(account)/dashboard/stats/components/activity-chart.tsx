...
export function ActivityChart({ stats }: ActivityChartProps) {
  ...
  const chartData = stats.map((child) => ({
    ...
    appUsage: child.app_usages,
    safeZones: child.safe_zones,
    devices: child.devices,
    total: child.call_logs + child.locations + child.app_usages,
  }));
  ...
  const activityDistribution = [
    {
      name: "Call Logs",
      value: stats.reduce((sum, child) => sum + child.call_logs, 0),
      color: "#3b82f6",
    },
    {
      name: "Locations",
      value: stats.reduce((sum, child) => sum + child.locations, 0),
      color: "#ef4444",
    },
    {
      name: "App Usage",
      value: stats.reduce((sum, child) => sum + child.app_usages, 0),
      color: "#f59e0b",
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:gap-6 w-full md:grid-cols-1">
          ...
          <CardContent className="flex justify-center items-center">
            <ChartContainer
              config={{
                ...
                appUsage: { label: "App Usage", color: "#f59e0b" },
              }}
              className="h-[220px] sm:h-[260px] md:h-[300px] lg:h-[360px] w-full max-w-4xl"
            >
        ...
        <Card>
          ...
          <CardContent className="flex justify-center items-center">
            <ChartContainer
              ...
              className="h-[220px] sm:h-[260px] md:h-[300px] lg:h-[360px] w-full max-w-3xl"
            >
...
function ChildDetailSheet({ child }: { child: any }) {
  ...
  const {
    ...
    error: detailsError,
  } = useChildDetails(isOpen ? child.child_id : "");
  const { trends, loading: trendsLoading } = useActivityTrends(
    isOpen ? child.child_id : ""
  );
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold">{child.app_usages}</span>
              </div>
      ...
      <SheetContent className="w-full sm:max-w-[640px] overflow-y-auto p-4 sm:p-6">
        <SheetHeader>
          ...
          <SheetDescription>
            Comprehensive monitoring data for {child.child_name}
          </SheetDescription>
        </SheetHeader>

        <div className="mt-6">
          <Tabs defaultValue="overview" className="w-full">
            <TabsList className="w-full overflow-x-auto flex gap-1 sm:grid sm:grid-cols-7 sm:gap-0">
              <TabsTrigger
              ...
              <TabsTrigger className="whitespace-nowrap shrink-0" value="calls">
                Calls
              </TabsTrigger>
              <TabsTrigger
                className="whitespace-nowrap shrink-0"
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">
                          {details.dailyStats.totalMessages}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          Messages Today
                        </p>
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-2xl font-bold">
                          {details.dailyStats.timeInSafeZones}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          Safe Zone Checks
                        </p>
                  ...
                  {trendsLoading ? (
                    ...
                    <ChartContainer
                      config={{
                        ...
                        locations: { label: "Locations", color: "#ef4444" },
                        safeZoneTime: {
                          label: "Safe Zone Time",
                          color: "#f59e0b",
                        },
                      }}
                      className="h-[250px] w-full max-w-4xl"
                    >
                      <ResponsiveContainer width="95%" height="100%">
              ...
              {detailsLoading ? (
                ...
                <Card>
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-2 text-red-500">
                      <AlertCircle className="h-4 w-4" />
                      <p>Error loading app usage: {detailsError}</p>
                    </div>
                  </CardContent>
                </Card>
              ) : (
                <AppUsage usages={details.appUsages} />
              )}
            </TabsContent>
            ...
            <TabsContent value="devices" className="space-y-4">
              {detailsLoading ? (
                ...
                <>
                  ...
                  <DeviceDiagnostics devices={details.devices} />
                </>
              )}
            </TabsContent>
