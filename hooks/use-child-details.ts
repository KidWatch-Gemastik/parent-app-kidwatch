        ...
        async function fetchChildDetails() {
            ...
            try {
                const [
                    ...
                    supabase
                        .from("devices")
                        .select("id, device_id, os, status, platform, ip_address, created_at")
                        .eq("child_id", childId)
                ...
                const activeDevices = devicesRes.data?.filter((device) => device.status === "online").length || 0
                ...
                setDetails({
                    ...
                    activityLogs: [], // Will be implemented when activity_logs table structure is defined
...
function generateSafetyAlerts(locations: any[], calls: any[], devices: any[]): SafetyAlert[] {
    ...
    if (recentUnsafeLocations.length > 5) {
        alerts.push({
            ...
            type: "location",
            message: `Anak telah berada di luar zona aman dalam jangka waktu yang lama.`,
            severity: "medium",
